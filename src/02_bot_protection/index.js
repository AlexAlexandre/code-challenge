...
exports.botProtection = function(data) {
    let commandAsString = data.join('');
    let commandsToFind, occurrences;

    for (let i = 0; i < data.length -2; i++) {
        commandsToFind = data[i] + '' + data[i +1] + data[i + 2];
        occurrences = commandAsString.match(new RegExp(commandsToFind, 'g') || []).length;
        if (occurrences > 1) {
            return true;
        }
    }
